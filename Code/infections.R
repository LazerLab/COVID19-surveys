...
out <- read_wave_month_table(here("Data/"))
...
quantile(method1_results_states_join_prerapid$cdf_estimate_diff, c(0.025, 0.975))
...
ggsave(here("Plots", "infections_full_plot.png"),
       full_plot, 
       height = 14, width = 10,
       bg = "white", dpi = 300)
...
ggsave(here("Plot", "infections_state.png"),
       plot_cdf_states, 
       height = 14, width = 12,
       bg = "white", dpi = 300)


## STATE DIFFS PLOT Figure S7
...
ggsave(here("Plot", "infections_state_diff.png"),
       plot_cdf_states_diff, 
       height = 14, width = 12,
       bg = "white", dpi = 300)
...
criteria_3 <- function(df) {
  df %>% 
    arrange(wave) %>%
    mutate(diff = estimate - lag(estimate, 1),
           moe_sum = std_error*1.96 + lag(std_error*1.96,1),
           crit_3 = case_when((diff < 0) & (abs(diff) > moe_sum) ~ F,
                              T ~ T)) 
  
}

criteria_3(method2_results)

state_crit3 <- method2_results_states %>%
  group_by(state_code) %>%
...
#### Figure S2: AT LEAST 1 INFECTION PLOT ####
...
ggsave(here("Plots", "at_least_1_infection.png"),
       height = 6, width = 9,
       bg = "white", dpi = 300)
...
ggsave(here("Plots", "trust_bw_infections.png"),
       trust_bw_plot,
       height = 7, width = 12,
       bg = "white", dpi = 300)
