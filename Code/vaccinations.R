...
cat("Project root:", here(), "\n If the above is not .../COVID19-surveys/, change directory to your project root by using setwd()")

source(here("Code", "functions_read_data.R"))
source(here("Code", "functions_analysis.R"))
...
## Comparing Ipsos and CSP estimates: Table S8
...
df_state <-  df_state_diff %>% 
  select(Location, Date, wave, moe, diff, State, n) %>% 
  mutate(`Difference within 95% CI` = case_when(moe >= abs(diff) ~ "Yes",
                                                moe < abs(diff) ~ "No"))


df_state_preboost <- df_state %>%
  filter(!is.na(diff), wave <= 19) 

df_state_CI <- df_state_diff %>%
  filter(!is.na(diff), wave <= 18) 

### get data driven 95% CI
quantile(df_state_CI$diff, c(0.025, 0.975))


df_state_all <- df_state %>%
  mutate(Period = case_when(wave <=19 ~ "Pre-boosters",
...
ggsave(here("Plots", "vax_full_plot.png"), 
       full_plot, 
       height = 14, width = 10,
       bg = "white", dpi = 300)
...
criteria_3 <- function(df) {
  df %>%
    mutate(diff = vax - lag(vax, 1),
           moe_sum = moe + lag(moe,1),
           crit_3 = case_when((diff < 0) & (abs(diff) > moe_sum) ~ F,
                              T ~ T)) 
  
}

criteria_3(csp_vax)
criteria_3(ipsos_vax)

state_crit3 <- csp_vax_state %>%
  rename(vax = csp_vax) %>%
  group_by(state_code) %>%
  group_modify(~ criteria_3(.x))

state_crit3 %>% filter(crit_3 == F)

state_fluct <- csp_vax_state %>%
  filter(wave > 19) %>%
  group_by(state_code) %>%
  summarise(sd = sd(csp_vax),
            mean = mean(csp_vax))

### SI state plot 1 Figure S5
...
ggsave(here("Plots", "cdc_diff_state_panels.png"),
       height = 14, width = 12,
       bg = "white", dpi = 300)


### state plot 2 Figure S4
...
ggsave(here("Plots", "state_panels.png"),
       height = 14, width = 12,
       bg = "white", dpi = 300)
...
df_state_preboost %>% filter(wave > 14, `Difference within 95% CI` == "No") %>% 
  filter(diff<0)
  


##### Table S4 with unadjusted CDC values

cdc_un <- read_csv(here("Data", "CDC", "COVID-19_Vaccinations_in_the_United_States_Jurisdiction.csv")) %>%
...
ggsave(here("Plots", "trust_bw_vax.png"),
       trust_bw_plot,
       height = 7, width = 12,
       bg = "white", dpi = 300)
