...
## VS corresponds to the participants recruited through Facebook
...
national_trends <- filter(cdc, Location == "US",
  ...
  bind_rows(mutate(ipsos_vax, `Data Source` = "Ipsos"),
            mutate(csp_vax_ps, `Data Source` = "CSP PureSpectrum"),
...
plot_national_trends <- ggplot(national_trends) + 
  ...
  geom_line(aes(x = Date,
                ...
                color = `Data Source`),
            lwd = 0.75) + 
  scale_color_manual(values=c('black', '#0072B2', '#CC79A7', '#D55E00'))+
  scale_shape_manual(values = c(19, 16, 16, 17)) +  # CDC=circle, both CSPs=square, Ipsos=triangle
  scale_size_manual(values = c(0.01, 0.4, 0.4, 0.4), guide = "none") +
...
ggsave("Plots/vaccination_rate_VS_PS.png", 
